...
export class brandController { 
    static addBrand = async (req, res) => {
     ...
     try {
         ...
         else return res.status(400).send({message: "Add brand Error In Database"})
     } catch (error) {
 ...
 static updateBrand = async (req, res) => {
    ...
    try {
        ...
        if(isExist.length>0){
            ...
            if (data) res.status(200).send({message: "abmin Successfully Updated"})
        }else return res.status(400).send({message: "admin With The Specified Id Does Not Exists"})
    } catch (error) {
  ...
  static deleteBrand = async (req, res) => {
   ...
   try {
       ...
       return res.status(404).send({message: "brand With The Specified Id Does Not Exists"})
   } catch (error) {
    console.log('error :>> ', error);
       return res.status(500).send({message: "Internal Server Error"})
   }
