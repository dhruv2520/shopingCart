...
export class productController {
    ...
    static addProduct = async (req, res) => {
        try {
            const { productName, categoryId,brandId, productPrize, description } = req.body;
            ...
            if (!productName || !categoryId ||!brandId || !productPrize || !description)
    ...
    static updateProduct = async (req, res) => {
        ...
        try {
            ...
            if (isExist.length > 0) {
                ...
                if (data) res.status(200).send({ message: "product Successfully Updated" })
            } else return res.status(400).send({ message: "product With The Specified Id Does Not Exists" })
        } catch (error) {
