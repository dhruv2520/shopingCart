...
export class addressController {
    ...
    static addAddress= async (req, res) => {
            ...
            const {userId,city,district,pincode,address}  = req.body
                ...
                if (data) res.status(200).send({message: " address Successfully Added"})
                else return res.status(400).send({message: "Add address Error In Database"})
            } catch (error) {
        ...
        static updateAddress = async (req, res) => {
            ...
            try {
                ...
                if(isExist.length>0){
                    ...
                    if (data) res.status(200).send({message: "address Successfully Updated"})
                }else return res.status(400).send({message: "address With The Specified Id Does Not Exists"})
            } catch (error) {
